...
class AutowareObjectsSubscriberPublisher(Node):
	def __init__(self):
		...
		self.infrastructure_objects_df_cache = pd.DataFrame(columns=self.entry_columns)
		...
		self.odometry_subscription = self.create_subscription(Odometry, \
                '/localization/kinematic_state', self.odometry_updater, 10)
       ...
       'd_x', 'd_y', 'd_z', 'duplicate']
		...
		self.ego_vehicle_ground_truth_px = 0
		self.ego_vehicle_ground_truth_py = 0
		self.ego_vehicle_ground_truth_pz = 0
		self.ego_vehicle_ground_truth_qx = 0
		self.ego_vehicle_ground_truth_qy = 0
		self.ego_vehicle_ground_truth_qz = 0
		self.ego_vehicle_ground_truth_qw = 0
		
	def odometry_updater(self, odometry_msg):
	...
	def frame_transform(self):
		try:
			...
			t = self.tf_buffer.lookup_transform('base_link', 'map', self.time_stamp)
	...
	def df_merger(self):
		if not(self.onboard_objects_df.empty) and not(self.infrastructure_objects_df.empty):
			...
			for n in range(len(self.infrastructure_objects_df)):
				adversary_x = self.infrastructure_objects_df['x'][n]
				adversary_y = self.infrastructure_objects_df['y'][n]
				r2 = self.infrastructure_objects_df.iloc[n]['d_y']/2
				euclidean_distance_with_ground_truth_ego_vehicle = math.sqrt(pow((adversary_x-self.ego_vehicle_ground_truth_px), 2) + pow((adversary_y-self.ego_vehicle_ground_truth_py), 2))
				if euclidean_distance_with_ground_truth_ego_vehicle < r2 + 1.8:
					#print('euclidean_distance_with_ground_truth_ego_vehicle: ', euclidean_distance_with_ground_truth_ego_vehicle)
					#print("Entering if condition")
					self.infrastructure_objects_df = self.infrastructure_objects_df.drop(n)
					self.infrastructure_objects_df = self.infrastructure_objects_df.reset_index(drop=True)
					break
			'''
			ego_vehicle_px = self.ego_vehicle_ground_truth_px
			ego_vehicle_py = self.ego_vehicle_ground_truth_py
			ego_vehicle_dx = 4.67
			ego_vehicle_dy = 1.81
			ego_vehicle_ox = self.ego_vehicle_ground_truth_qx
			ego_vehicle_oy = self.ego_vehicle_ground_truth_qy
			ego_vehicle_oz = self.ego_vehicle_ground_truth_qz
			ego_vehicle_ow = self.ego_vehicle_ground_truth_qw
			ego_heading_angle = self.euler_from_quaternion(ego_vehicle_ox, ego_vehicle_oy, ego_vehicle_oz, ego_vehicle_ow)
			yaw_angle = ego_heading_angle * 180 / math.pi
			ego_centerpoints = self.find_centerline_points_for_overlap_verification(ego_vehicle_px, ego_vehicle_py, ego_vehicle_dx, ego_vehicle_dy, yaw_angle)

			ego_vehicle_duplicate_found = 0

			for n in range(len(self.infrastructure_objects_df)):
				#print("Entering ONE")
				if ego_vehicle_duplicate_found == 1:
					break
				adversary_px = self.infrastructure_objects_df['x'][n]
				adversary_py = self.infrastructure_objects_df['y'][n]
				adversary_dx = self.infrastructure_objects_df.iloc[n]['d_x']
				adversary_dy = self.infrastructure_objects_df.iloc[n]['d_y']
				adversary_ox = self.infrastructure_objects_df['o_x'][n]
				adversary_oy = self.infrastructure_objects_df['o_y'][n]
				adversary_oz = self.infrastructure_objects_df['o_z'][n]
				adversary_ow = self.infrastructure_objects_df['o_w'][n]
				adversary_heading_angle = self.euler_from_quaternion(adversary_ox, adversary_oy, adversary_oz, adversary_ow)
				yaw_angle = adversary_heading_angle * 180 / math.pi
				adversary_half_width = self.infrastructure_objects_df.iloc[n]['d_y']/2

				adversary_centerpoints = self.find_centerline_points_for_overlap_verification(adversary_px, adversary_py, adversary_dx, adversary_dy, yaw_angle)

				euclidean_distance_between_ego_and_adversary_vehicle = \
					math.sqrt(pow((adversary_px-ego_vehicle_px), 2) + pow((adversary_py-ego_vehicle_py), 2))
				#print("euclidean_distance_between_ego_and_adversary_vehicle: ", euclidean_distance_between_ego_and_adversary_vehicle)
				
				if euclidean_distance_between_ego_and_adversary_vehicle < ego_vehicle_dx/2 + adversary_dx/2:
					#print("Entering TWO")
					number_of_centerline_points_in_ego_vehicle = 10 - np.count_nonzero(np.isnan(ego_centerpoints))
					number_of_centerline_points_in_adversary_vehicle = 10 - np.count_nonzero(np.isnan(adversary_centerpoints))
					for ec in range(0, int(number_of_centerline_points_in_ego_vehicle/2)):
						#print("Entering THREE")
						if ego_vehicle_duplicate_found == 1:
							break
						x1 = ego_centerpoints[ec][0]
						y1 = ego_centerpoints[ec][1]
						for ac in range(0, int(number_of_centerline_points_in_adversary_vehicle/2)):
							#print("Entering FOUR")
							x2 = adversary_centerpoints[ac][0]
							y2 = adversary_centerpoints[ac][1]
							euclidean_distance = math.sqrt(pow((x2-x1), 2) + pow((y2-y1), 2))
							if euclidean_distance < ego_vehicle_dy/2 + adversary_half_width:
								self.infrastructure_objects_df = self.infrastructure_objects_df.drop(n)
								self.infrastructure_objects_df = self.infrastructure_objects_df.reset_index(drop=True)
								ego_vehicle_duplicate_found = 1
								break
				
			#print("right after if condition")
			#while self.infrastructure_objects_df.empty:
			#	break
			
			self.frame_transform()
			#self.infrastructure_objects_df = self.infrastructure_objects_df_cache
			...
			for m in range(len(transformed_quaternion)):
				#print("transformed_quaternion: ", transformed_quaternion)
				#print("len of transformed_quaternion: ", len(transformed_quaternion))
            ...
            # source 0 refers to on-board objects
			# source 1 referes to infrastructure objects
			#print("right before processing existence of duplicates")
			...
			for i in range(len(self.objects_df)):
				#print("processing existence of duplicates")
			...
			print("Time difference from input to output: ", time_difference*1000, " ms")
			self.objects_df = pd.DataFrame(columns=self.columns)
			self.onboard_objects_df = pd.DataFrame(columns=self.entry_columns)
			self.infrastructure_objects_df = pd.DataFrame(columns=self.entry_columns)
			#exit()
